 ...
 * <p>认证优先级:
 * <ol>
 *     <li>网关注入的 {@code X-User-Id} Header (已由网关验证 JWT)</li>
 *     <li>{@code Authorization: Bearer} JWT 直接验证 (独立访问模式)</li>
 * </ol>
 *
 * <p>认证成功后将 {@link AuthUserDetails} 设入 {@link SecurityContextHolder}.
 * 若两种认证方式均不满足, 则不设置认证信息, 由 Spring Security 后续处理
 * (通常返回 401 Unauthorized).
 *
 * <p>线程安全: 本过滤器为无状态对象, 可安全注册为 Spring Bean 单例.
 */
public class AuthHeaderFilter extends OncePerRequestFilter {
     ...
     *
     * <p>当服务始终部署在网关之后时可传 null, 此时仅从网关 Header 构建认证信息.
     */
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (userDetails == null) {
            ...
            if (authHeader != null && authHeader.startsWith(BEARER_PREFIX) && jwtValidator != null) {
    ...
    private AuthUserDetails buildFromHeaders(HttpServletRequest request) {
        try {
            ...
            log.debug("从网关 Header 构建认证信息: userId={}, username={}",
    ...
    private AuthUserDetails buildFromJwt(String token) {
        try {
            AuthUserDetails details = jwtValidator.toUserDetails(token);
            log.debug("从 JWT 构建认证信息: userId={}, username={}",
                    details.getUserId(), details.getUsername());
            return details;
        } catch (Exception e) {
