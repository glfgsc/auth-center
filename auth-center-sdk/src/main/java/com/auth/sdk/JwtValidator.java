 ...
 * <p>公钥缓存策略:
 * <ul>
 *     <li>正常情况下每 {@value #REFRESH_INTERVAL_MINUTES} 分钟自动刷新</li>
 *     <li>验证失败时立即刷新一次 (应对密钥轮换场景)</li>
 *     <li>JWKS 拉取失败时降级使用缓存中的旧公钥, 避免全局不可用</li>
 * </ul>
 *
 * <p>线程安全: 使用 {@code volatile} + 同步块保护公钥缓存, 可在多线程环境中共享使用.
 */
public class JwtValidator {
     ...
     * 构造 JWT 验证器.
     *
     * @param authCenterBaseUrl Auth Center 根 URL, 如 {@code http://auth-center:8090}
     *                          末尾不需要斜杠
     */
    public JwtValidator(String authCenterBaseUrl) {
        // 去除末尾斜杠
        String baseUrl = authCenterBaseUrl.endsWith("/")
     ...
     * <p>验证流程:
     * <ol>
     *     <li>使用缓存的公钥验证签名和有效期</li>
     *     <li>若签名失败, 立即刷新公钥后重试一次 (应对密钥轮换)</li>
     *     <li>重试仍失败则抛出异常</li>
     * </ol>
     *
        ...
        synchronized (fetchLock) {
            ...
            try {
                ...
                throw new IllegalStateException(
                        "无法获取 RSA 公钥且无缓存可用, JWKS URL: " + jwksUrl, e);
            }
        }
    }
     ...
     * <p>解析流程:
     * <ol>
     *     <li>HTTP GET 请求 JWKS URL</li>
     *     <li>解析 JSON, 提取 {@code keys} 数组中第一个 RSA 密钥</li>
     *     <li>从 {@code n} (模数) 和 {@code e} (指数) 构造 {@link RSAPublicKey}</li>
     * </ol>
     *
     * @return RSA 公钥
